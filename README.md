...
- 브렌트 유가는 더 이상 양방향 변동성 신호로 처리하지 않습니다.
- 한국 주식시장 관점에서 유가 하락은 비용·물가 부담 완화 요인이므로, `BRENT`는 `positive_tail` 방식으로 바꿨습니다.
- `positive_tail`은 표준화된 z-score가 양수일 때만 위험 신호를 남기고, 음수 z-score는 0으로 처리합니다.
- 따라서 유가 하락 모멘텀은 공급환경 채널과 RMS 종합 스트레스 점수에 악재로 반영되지 않습니다.
